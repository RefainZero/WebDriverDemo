	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult suiteResult : suiteResults.values()) {
				...
				Set<ITestResult> method = passedTests.getAllResults();
				//移除添加设置标题串联测试类，报告标题不计入测试结果集合
				for(ITestResult rs:method){
					String methodName = rs.getMethod().getMethodName();
					if(methodName.equals("titleSetTing")){
						passedTests.removeResult(rs.getMethod());
					}
				}
